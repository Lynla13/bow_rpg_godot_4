...
@onready var base_arrow_collide = $FiniteStateMachine/Base_Arrow_Collide
...
func _ready():
	#fire
	arrow_fire._collide.connect (fsm._change_state.bind(base_arrow_collide))
	arrow_fire._out.connect (fsm._change_state.bind(arrow_out))
	#collide
	base_arrow_collide._out.connect (fsm._change_state.bind(arrow_out))
	#out
